...
```{r, warning=FALSE, cache=TRUE, results='asis', tidy=FALSE, echo=FALSE, message=FALSE, eval=FALSE}
options(gvis.plot.tag = 'html')
G1<-gvisMotionChart(mydata, idvar="country", timevar="year", xvar=ind[1], yvar=ind[2], sizevar=ind[1], colorvar=ind[2], date.format="%Y", options=list(height=300))
G2<-gvisGeoChart(mydata[mydata$year==max(mydata$year),],locationvar="country", colorvar=ind[1], options=list(width=200, height=150, colorAxis="{colors:['#FFFFFF', '#0000FF']}")) 
G3<- gvisGeoChart(mydata[mydata$year==max(mydata$year),],locationvar="country", colorvar=ind[2], options=list(width=200, height=150 )) 
G23<- gvisMerge(G2,G3, horizontal=F);G123<- gvisMerge(G23,G1, horizontal=T)
plot(G123)
```
